...
export default function TabBar() {
  ...
  return (
    <>
      <div
  className="flex items-center border-b border-zinc-800 bg-zinc-900 overflow-x-auto whitespace-nowrap no-scrollbar"
>
  {tabs.map((tab) => (
    <div
      key={tab.id}
      ...
      className={`group px-4 py-2 flex items-center gap-2 cursor-pointer flex-shrink-0 ${
        activeTabId === tab.id
  ...
  <button
    onClick={addTab}
    className="px-3 py-2 text-zinc-400 hover:text-white flex-shrink-0"
  >
    +
  </button>
</div>
